                        ...
                        <div class="space-x-3 flex justify-between"> 
                            ...
                            <div class="flex-1 space-y-3">
                                ...
                                <select name="condicion_id"  id="colorSelectedit" class="bg-white-50 border border-gray-200 text-gray-900 text-sm rounded focus:ring-blue-500 focus:border-blue-500 block w-64 h-10 p-2.5 dark:focus:border-blue-500">
                                    <option type="text" value="PENTIENDE">Pendiente</option>
...
<div class="space-x-3 flex justify-between">  
        <div class="flex-1 space-y-3">
                ...
                <input type="text" name="observaciones" id="observacionesedit"
                class="bg-gray-50 border outline-none border-gray-300 text-gray-900 text-sm rounded focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                    placeholder="Comentarios acerca del acta" value=".">
        </div>
...
@endphp
    ...
    <script>
                function editar_acta(id,numero,estado,inspector_id,infraccion_id,retencion,conductor_dni,conductor_nombres,conductor_apellidos,conductor_licencia,categoria,estadolicencia,empresa_id,vehiculo_placa,ruta2,observacion)
                    {
...
//
                    ...
                    function eliminarActa(id) {
                        // Aquí puedes implementar la lógica para confirmar la eliminación y enviar la solicitud al servidor
                        var confirmar = confirm("¿Estás seguro de que deseas eliminar este registro?");
                        
                        if (confirmar) {
                            // Realiza la solicitud AJAX o redirige a la ruta de eliminación
                            // Puedes utilizar fetch, axios o jQuery.ajax según tu preferencia
                            // Ejemplo con fetch:
                            fetch(`/acta/${id}`, {
                                method: 'DELETE',
                                headers: {
                            ...
                            .then(data => {
                                if (data.success) {
                                    console.log('Operativo eliminado. Recargando la página...');
                                    location.reload();
                                } else {
